...
function getNumberPhone(a)
    local b= exports.ghmattimysql:scalarSync("SELECT users.phone_number FROM users WHERE users.identifier = @identifier",{["@identifier"]=a})
    if b~=nil then 
        return b 
    end;
    return nil 
...
function getIdentifierByPhoneNumber(phone_number) 
    local result = exports.ghmattimysql:executeSync("SELECT users.identifier FROM users WHERE users.phone_number = @phone_number", {
        ...
        if user == nil then 
            ...
            exports.ghmattimysql:scalar("SELECT identifier FROM twitter_accounts WHERE identifier = @identifier", {
                ...
                if result ~= nil then
                   ...
                   exports.ghmattimysql:execute('INSERT IGNORE INTO twitter_accounts (`identifier`, `username`) VALUES(@identifier, @username)', {
					['@identifier'] = identifier,
...
function getPlayerID(source)
    ...
    local player = getIdentifiant(xPlayer)
    return player
end
...
RegisterCommand('telfix', function(source)
	...
	TriggerClientEvent('sarisayfafix', source)
end)
