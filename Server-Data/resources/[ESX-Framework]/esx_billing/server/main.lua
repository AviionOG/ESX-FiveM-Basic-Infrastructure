		...
		TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
				exports.ghmattimysql:execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					...
					TriggerClientEvent('Kibra:Notify', xTarget.source, 'Fatura', 'Yeni Bir Fatura Aldınız!', 3000, 'info')
				end)
			else
				exports.ghmattimysql:execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					['@identifier'] = xTarget.identifier,
			...
			if result[1].target_type == 'player' then
				if xTarget then
					if xPlayer.getMoney() >= amount then
						exports.ghmattimysql:execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Fatura', amount..' tutarındaki faturanız ödendi', 3000, 'success')
								TriggerClientEvent('Kibra:Notify', xTarget.source, 'Fatura', amount..' tutarında kestiğiniz fatura ödendi', 3000, 'success')
							end
					...
					elseif xPlayer.getAccount('bank').money >= amount then
						...
						TriggerClientEvent('Kibra:Notify', xTarget.source, 'Fatura','Fatura keseceğiniz kişinin bunu karşılayacak parası yok!', 3000, 'error')
						TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Fatura','Paranız Yetersiz!', 3000, 'error')
						cb()
					end
				else
					TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Fatura','Oyuncu Şehirde Değil!', 3000, 'error')
					cb()
				end
			else
