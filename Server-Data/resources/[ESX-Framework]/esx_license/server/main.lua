...
function GetLicense(type, cb)
	exports.ghmattimysql:execute('SELECT label FROM licenses WHERE type = @type', {
		['@type'] = type
	}, function(result)
		local data = {
			type  = type,
...
function GetLicenses(target, cb)
	...
	exports.ghmattimysql:execute('SELECT type FROM user_licenses WHERE owner = @owner', {
		['@owner'] = xPlayer.identifier
	}, function(result)
...
function GetLicensesList(cb)
	exports.ghmattimysql:execute('SELECT type, label FROM licenses', {
		['@type'] = type
	}, function(result)
		local licenses = {}
