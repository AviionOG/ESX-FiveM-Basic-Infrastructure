...
AddEventHandler("almez:yedek", function(motelId)
        ...
        TriggerClientEvent('Kibra:Notify', src, 'Yetersiz!', 'Yeterince paran yok!', 3000, 'error')
    end
end)
...
AddEventHandler('almez-motels:server:toggleDoor', function(motelId, doorId, state)
    ...
    if state == not locked then
        TriggerClientEvent('Kibra:Notify', source, 'Başarısız', motelId..' numaralı kapı kilitlendi.', 3000, 'error')
    else
        TriggerClientEvent('Kibra:Notify', source, 'Başarılı', motelId..' numaralı kapı açıldı!', 3000, 'success')
    end
end)
...
AddEventHandler('almez-motels:server:toggleStash', function(motelId, doorId, state)
    ...
    if state == not locked then
        TriggerClientEvent('Kibra:Notify', source, 'Başarılı', motelId..' numaralı depo kilitlendi!', 3000, 'error')
    else
        TriggerClientEvent('Kibra:Notify', source, 'Başarılı', motelId..' numaralı depo açıldı!', 3000, 'success')
    end
end)
...
AddEventHandler('almez-motels:server:buyMotel', function(motelId)
    ...
    if money >= price then
       exports.ghmattimysql:execute('UPDATE almez_motels SET `owner` = @owner WHERE `key` = @key', {
        ...
        TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarılı', motelId..' numaralı odayı satın aldın', 3000, 'success')
        local info = {}
        info.motel = motelId
        xPlayer.addInventoryItem('motelkeys', 1, false, info)
    else
        TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', ' Paran Yetersiz!', 3000, 'error')
    end
end)
            ...
            local motelitem = almez[motelId].info.data.getItem(name)
                if item.count >= count then
                    if ((motelitem.weight * count) + almez[motelId].info.data.currentweight) <= Cfg.MotelWeightLimit then
                        ...
                        TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', ' Dolabınızda bu kadar eşyaya yer yok!', 3000, 'error')

                    end
                else
                    TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', ' Bu kadar eşyaya sahip değilsin!', 3000, 'error')
                end
    elseif type == "item_weapon" then
        ...
        local motelweapon = almez[motelId].info.data.getWeapon(name)
                if wep then
                    if motelweapon.ammo == 0 then
                        if (motelweapon.weight + almez[motelId].info.data.currentweight) <= Cfg.MotelWeightLimit then
                            ...
                            TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', ' Bu silahı koymak için yer yok!', 3000, 'error')
                        end
                            else
                                xPlayer.removeWeapon(name)
                                almez[motelId].info.data.addWeapon(name, count)
                    ...
                    elseif not wep then
                        TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', ' Üstünüzde Silah Yok!', 3000, 'error')
                    end
            ...
            if black >= count then
                ...
                TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', 'Üzerinizde bu kadar yok!', 3000, 'error')
            end
    end
end)
        ...
        local playeritem = xPlayer.getInventoryItem(name)
            ...
            if motelitem.count >= count then
                if xPlayer.canCarryItem(name, count) then
                    ...
                    TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', 'Bu eşyadan daha fazla taşıyamazsınız!', 3000, 'error')
                end
            else
                TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', 'Evinizde bu kadar eşya yok!', 3000, 'error')
            end
    elseif type == "item_weapon" then
            local wep = xPlayer.hasWeapon(name)
            local motelweapon = almez[motelId].info.data.getWeapon(name)
            ...
            if wep then
                if motelweapon.ammo >= count then
                    ...
                    TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', 'Evinizde bu kadar mühimmat yok!', 3000, 'error')
                end
            elseif not wep then
                if xPlayer.canCarryItem(name, 1) then
                    ...
                    TriggerClientEvent('Kibra:Notify', xPlayer.source, 'Başarısız', 'Bu silahı taşıyamazsın!', 3000, 'error')
                end
            end
    elseif type == "item_account" then
