...
Citizen.CreateThread(function()
	for k, v in pairs(Config.HeliGarages) do
		local gblip = AddBlipForCoord(v.positions.menu.position)
		SetBlipSprite(gblip, 43)
		SetBlipDisplay(gblip, 4)
		SetBlipScale (gblip, 0.4)
		SetBlipColour(gblip, 3)
		SetBlipAsShortRange(gblip, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Helikopter Garajı")
		EndTextCommandSetBlipName(gblip)
	end
end)
...
Citizen.CreateThread(function()
	for k, v in pairs(Config.BoatGarages) do
		local gblip = AddBlipForCoord(v.positions.menu.position)
		SetBlipSprite(gblip, 427)
		SetBlipDisplay(gblip, 4)
		SetBlipScale (gblip, 0.4)
		SetBlipColour(gblip, 3)
		SetBlipAsShortRange(gblip, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Tekne Garajı")
		EndTextCommandSetBlipName(gblip)
	end
end)
				...
				if distance <= 10.0 then
					...
					if IsPedInAnyVehicle(PlayerPedId()) then
						...
						if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
							if i == "vehicle" then
								if distance <= 10.0 then
									DrawMarker(6, j.position - vector3(0.0, 0.0, 0.925), 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 5.0, 1.0, 5.0, 0, 150, 150, 100, false, true, 2, false, false, false, false)
							...
							elseif i == "menu" then
								if distance <= 5.0 then
									DrawMarker(6, j.position - vector3(0.0, 0.0, 0.925), 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 1.5, 1.5, 1.5, 0, 150, 150, 100, false, true, 2, false, false, false, false)
								end
							end
						end
					else
			...
			for i, j in pairs(v.positions) do
				...
				if distance <= 10.0 then
					...
					if not IsPedInAnyVehicle(PlayerPedId()) then
						if i == "impmenu" then
							if distance <= 5.0 then
								DrawMarker(6, j.position - vector3(0.0, 0.0, 0.925), 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 1.5, 1.5, 1.5, 255, 0, 0, 100, false, true, 2, false, false, false, false)
								if distance <= 3.5 then
...
function SpawnImpVehicle(vehicleProps)
	...
	ESX.TriggerServerCallback('m3:garage:plateCheck', function(result)
		if not result then
			...
			local pedhash = GetHashKey('s_m_y_valet_01')
...
function SetVehicleProperties(vehicle, vehicleProps)
    ...
    exports["LegacyFuel"]:SetFuel(vehicle, vehicleProps.fuelLevel) -- legacy fuel esx
